...
def calculate_metrics(img_r, img_s, forward_tp_gt, inverse_tp_gt, forward_tp_pred, inverse_tp_pred, mask_r, mask_s, cfg, metric_results, h=256, w=256):
    ...
    assert not torch.isnan(forward_tp_pred).any(), "forward_tp_pred contains NaNs"
    ...
    bbox_s = apply_normalized_affine_to_polygon(bbox_r, forward_tp_gt.cpu().squeeze().numpy().reshape(2, 3), h, w)
    bbox_r_registered = apply_normalized_affine_to_polygon(bbox_s, inverse_tp_pred.cpu().squeeze().numpy().reshape(2,3), h, w)
    bbox_s_registered = apply_normalized_affine_to_polygon(bbox_r, forward_tp_pred.cpu().squeeze().numpy().reshape(2,3), h, w)
    ...
    for name, options in cfg['metrics'].items():
        ...
        if name in ('rmse', 'mpd', 'iou'):
            ...
            metric_results[name] += METRIC_REGISTRY.get(options['type'])(forward_tp_gt.cpu().squeeze().numpy().reshape(2,3), forward_tp_pred.cpu().squeeze().numpy().reshape(2,3))
            metric_results[name] += METRIC_REGISTRY.get(options['type'])(inverse_tp_gt.cpu().squeeze().numpy().reshape(2,3), inverse_tp_pred.cpu().squeeze().numpy().reshape(2,3))
        elif name in ('mi', 're'):
