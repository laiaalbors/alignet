...
class DREAMCSVDataset(BaseDataset):
    def __init__(self, root_path, img_size, train=True, cfg=None):
        ...
        del records
        self.labels = list(self.label_to_idx.values())
    ...
    def __getitem__(self, index):
        row = self.df[index]
        path_1 = os.path.join(self.data_path, row['zone'], row['label'], row['image_path'])
        path_2 = os.path.join(self.data_path, row['zone'], row['label2' if self.mode == "multi" else 'label'], row['image_path'])
        ...
        label_1 = torch.tensor(self.label_to_idx[row['label']], dtype=torch.long)
        label_2 = torch.tensor(self.label_to_idx[row['label2' if self.mode == "multi" else 'label']], dtype=torch.long)
        ...
        # Crop images
        if "offset1" not in self.df_columns:
            ...
            contained = True
            image_1_crop = F.crop(image_1, offset1, offset2, h, w)
            transformed_image_2_crop = F.crop(transformed_image_2, offset1, offset2, h, w)
