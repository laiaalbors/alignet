...
This repository contains the details to reproduce the work presented in [this paper](www.link-to-the-workshop-paper.com).
...
N=$(grep -cve '^[[:space:]]*$' -e '^[[:space:]]*#' scripts/test_commands_mdas_affine.txt)
sbatch --array=1-${N}%3 scripts/test_array.sh scripts/test_commands_mdas_affine.txt

# Evaluate every individual pair in DREAM
N=$(grep -cve '^[[:space:]]*$' -e '^[[:space:]]*#' scripts/test_commands_dream_affine.txt)
sbatch --array=1-${N}%3 scripts/test_array.sh scripts/test_commands_dream_affine.txt
```
